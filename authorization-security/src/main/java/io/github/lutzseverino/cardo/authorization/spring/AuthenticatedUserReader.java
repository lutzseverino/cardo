...
public class AuthenticatedUserReader {
  ...
  public AuthenticatedUser currentUser() {
    ...
    throw invalidToken("Missing Cardo JWT authentication.");
  }
  ...
  private UUID identityUserId(JwtAuthenticationToken authentication) {
    String value = authentication.getToken().getClaimAsString(CardoJwtClaims.IDENTITY_USER_ID);
    if (value == null || value.isBlank()) {
      throw invalidToken("Missing Cardo identity user claim.");
    }
    try {
      return UUID.fromString(value);
    } catch (IllegalArgumentException exception) {
      throw invalidToken("Invalid Cardo identity user claim.");
    }
  }
