  ...
  SecurityFilterChain referenceIdentityWorkflowSecurity(
    ...
    http.securityMatcher(workflowRequests())
        ...
        .oauth2ResourceServer(
            oauth ->
                oauth
                    .bearerTokenResolver(new ReferenceIdentitySessionCookieResolver(sessionCookie))
                    .jwt(
                        jwt ->
                            jwt.decoder(identitySessions)
...
final class ReferenceIdentitySessionCookieResolver implements BearerTokenResolver {
  ...
  public String resolve(HttpServletRequest request) {
    if (request.getHeader(HttpHeaders.AUTHORIZATION) != null || request.getCookies() == null) {
      return null;
    }
