...
class StripeBillingProviderTest {
  ...
  @Test
  void createUsesTheOperationAsMetadataCorrelationAndIdempotencySuffix() throws Exception {
    ...
    verify(responseGetter).request(request.capture(), eq((Type) Customer.class));
    assertThat(request.getValue().getOptions().getIdempotencyKey())
        .isEqualTo("cardo-billing-customer-v2:" + operationId)
        .hasSizeLessThanOrEqualTo(255);
    assertThat(request.getValue().getParams().get("metadata"))
        .isInstanceOfSatisfying(
            Map.class,
            metadata -> {
              assertThat(metadata)
                  .containsEntry("subject_id", subjectId.toString())
                  .containsEntry(
                      StripeBillingProvider.PROVISIONING_METADATA_KEY, operationId.toString());
            });
  }
